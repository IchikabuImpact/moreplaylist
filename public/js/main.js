...
$(document).ready(function() {
    ...
    const feedUrl = getParameterByName('feed_url');

    if (feedUrl) {
        const playlistIdMatch = feedUrl.match(/list=([^&]+)/);
        if (playlistIdMatch) {
            const playlistId = playlistIdMatch[1];
            console.log('feed_url detected, playlistId:', playlistId);
            // 再生リストIDに基づいて動画を取得
            fetchVideosByPlaylistId(playlistId);
        } else {
            console.error('Invalid feed_url format');
        }
    } else {
        console.log('No feed_url, using default keyword');
        ytVideoApp.feeds_from_keyword('Lo-Fi');
    }
    ...
    function fetchVideosByPlaylistId(playlistId) {
        $.ajax({
            url: '/api/playlist-videos',
            method: 'GET',
            data: { playlistId: playlistId },
